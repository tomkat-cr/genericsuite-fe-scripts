...
if [ "${1:-}" = "create" ]; then
    bash "${BACKEND_PATH}/node_modules/genericsuite-be-scripts/scripts/local_ssl_certs_creation.sh" "${APP_LOCAL_DOMAIN_NAME}" "${BACKEND_PATH}"
elif [ "${1:-}" = "copy" ]; then
    bash "${BACKEND_PATH}/node_modules/genericsuite-be-scripts/scripts/local_ssl_certs_copy.sh" "${APP_LOCAL_DOMAIN_NAME}" "${FRONTEND_PATH}" "${BACKEND_PATH}"
else
